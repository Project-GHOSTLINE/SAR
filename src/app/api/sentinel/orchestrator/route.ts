...
function getNewAchievements(old: any, updated: any): string[] {
  ...
  if (old.missions_completed < 50 && updated.missions_completed >= 50) {
    achievements.push('🏆 50 Missions Complete');
  }
  if (old.missions_completed < 100 && updated.missions_completed >= 100) {
    achievements.push('🏆 100 Missions Master');
  }

  // Threat hunter
  if (old.threats_eliminated < 100 && updated.threats_eliminated >= 100) {
    achievements.push('⚔️ Threat Hunter - 100 Kills');
  }

  // Level milestones
  if (updated.level === 10) {
    achievements.push('⭐ Level 10 - Elite Status');
  }
  if (updated.level === 20) {
    achievements.push('⭐ Level 20 - Legendary');
  }
