-- 021_migrate_emails_envoyes_to_communications.sql (FIXED)
...
WHERE ee.message_id IS NOT NULL
  -- Vérifier que cette communication n'existe pas déjà
  AND NOT EXISTS (
    SELECT 1 FROM public.communications com
    WHERE com.provider = 'emails_envoyes'
      AND com.provider_message_id = ee.message_id::text
  );
